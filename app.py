# app.py
# Streamlit app for kebele-level PPS village sampling + HH systematic sampling (Eligible / Non-eligible)
# Input A: Villages -> Woreda | Kebele | Village | HHs
# Input B: Roster   -> Woreda | Kebele | Village | Eligibility | HH_ID | Household Head Name | Phone | Other ID
# Optional: Quotas  -> Woreda | Kebele | Village | nE | nNE
...
st.set_page_config(
    page_title="Kebele-level PPS Village & HH Sampler",
    page_icon=PAGE_ICON,
    layout="wide"
)

# ---------------------------------------------------------
# Branded header (logo + title) + Helper Drawer
# ---------------------------------------------------------
def render_header():
...
with st.sidebar:
    ...
    with st.expander("📘 Help & Guide", expanded=False):
        st.markdown(
            """
            **What this app does**  
            1) **PPS village selection** at kebele level (Systematic or Independent).  
            2) **Systematic household sampling** within each sampled village, for **Eligible** and **Non-eligible**.
            ...
            **Outputs**  
            • `Sampled_Villages.csv / .xlsx (Diagnostics)`  
            • `HH_Sample.csv / .xlsx (HH_Summary)`

            **Tips**  
            • Use **Systematic PPS** for no duplicates and even spread.  
            • Set a **Random seed** for reproducibility.  
            • For large rosters, filter to target woredas/kebeles for speed.

            **User Guide (PDF)** – add your link here.
            """
        )
...
with st.sidebar:
    ...
    method = st.selectbox("PPS Method", ["Systematic", "Independent"], index=0,
                          help="Systematic avoids duplicates; Independent uses random draws.")
...
if uploaded is not None:
    try:
        ...
        with col1:
            ...
            st.markdown("**Kebele → #Villages**"); st.dataframe(kebele_counts, use_container_width=True, height=240)
        with col2:
            hh_sum = df.groupby(["woreda", "kebele"], as_index=False)["hhs"].sum()
            hh_sum.rename(columns={"hhs": "Total HHs"}, inplace=True)
            st.markdown("**Kebele → Total HHs**"); st.dataframe(hh_sum, use_container_width=True, height=240)
        st.markdown("---")
...
# ---------------------------------------------------------
# NEW: Household Systematic Sampling helpers
# ---------------------------------------------------------
def read_roster(file) -> pd.DataFrame:
    name = getattr(file, "name", "uploaded"); ext = str(name).lower().split(".")[-1]
    if ext in ["xlsx", "xls"]:
...
def read_quotas(file) -> pd.DataFrame:
    ...
    if missing:
        raise ValueError(f"Quotas missing columns: {missing}")
    for c in ["woreda", "kebele", "village"]:
        q[c] = q[c].astype(str).str.strip()
    for c in ["nE", "nNE"]:
        q[c] = pd.to_numeric(q[c], errors="coerce").fillna(0).astype(int).clip(lower=0)
...
def sample_households_systematic(roster: pd.DataFrame, sampled_villages: pd.DataFrame,
                                  default_nE: int, default_nNE: int, order_by: str,
                                  quotas: pd.DataFrame | None, seed_base: str):
    sv = sampled_villages.copy()
    sv.columns = [c.lower() for c in sv.columns]
    sv = sv.rename(columns={"village / ea": "village"})
    for c in ["woreda", "kebele", "village"]:
        if c in sv.columns: sv[c] = sv[c].astype(str).str.strip()
    ...
    out_rows = []; sum_rows = []
    for _, vrow in sv[["woreda","kebele","village"]].drop_duplicates().iterrows():
        ...
        try:
            ...
            if run_hh:
                hh_sample, hh_summary = sample_households_systematic(
                    roster=roster_df, sampled_villages=sampled, default_nE=default_nE,
                    default_nNE=default_nNE, order_by=order_by.strip().lower(), quotas=quotas_df, seed_base=hh_seed
                )
...
st.markdown(
    ...
    <div style="color:gray; font-size:0.9em;">© WFP Ethiopia – Somali Region (Jijiga AO) | PPS village & household sampling utility</div>
    """,
    unsafe_allow_html=True
)
